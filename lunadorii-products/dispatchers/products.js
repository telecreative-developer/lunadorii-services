...
const { productsDefinition, singleProductDefinition, productBrandsDefinition, productCategoriesDefinition, productSubcategoriesDefinition } = require('../definitions/products')
...
exports.getSingleProduct = (product_id) => {
	return knex('products')
		...
		.then(response => NestHydrationJS.nest(response, singleProductDefinition))
		.then(response => response.map(res => ({
			...res,
			product_rate: res.reviews.map(r => r.review_rate).reduce((a, b) => a+b) / res.reviews.length
		})))
