...
exports.authFacebook = (data) => {
	return knex('users')
		...
		.then(response => {
			if(response.length) {
				const check = !!response.filter(rescheck => rescheck.provider === 'facebook').length
				if(check) {
				...
				return fetch(`https://graph.facebook.com/${data.id}?fields=id,first_name,last_name,email,picture&access_token=${data.accessToken}`)
					.then(res => res.json())
					.then(res => {
						return knex('users')
							.insert({
